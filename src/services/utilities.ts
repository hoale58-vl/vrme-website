...
export const isHttpErrorResponseModel = (response: any): response is HttpErrorResponseModel =>
...
export default class HttpUtility {
    ...
    static async _request(restRequest: any, config: any) {
        if (!restRequest.url) {
            console.error(
                `Received ${restRequest.url as string} which is invalid for a endpoint url`
            );
        }
                ...
                axios({
                    ...
                    headers: {
                        ...
                        Authorization: loggedInUser
                            ? 'Bearer ' + String(JSON.parse(loggedInUser).token)
                            : '',
                        ...config?.headers,
                    },
                }),
    ...
    static _fillInErrorWithDefaults(error: any, request: any): HttpErrorResponseModel {
        return {
            id: uuidv4(),
            status: error.status || 0,
            message: error.message || 'Error requesting data',
            errors: error.errors.length ? error.errors.filter(Boolean) : ['Error requesting data'],
...
export function truncateLongHexString(hexString: string, sliceLength: number = 6) {
    return `${hexString.slice(0, sliceLength)}...${hexString.slice(-sliceLength)}`;
}
